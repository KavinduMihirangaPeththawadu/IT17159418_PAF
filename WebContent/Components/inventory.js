...
$(document).on("click", ".btnRemove", function(event) {
	$.ajax({
		url : "inventoryAPI",
		type : "DELETE",
		data : "inventoryId=" + $(this).data("inventoryid"),
		dataType : "text",
		complete : function(response, status) 
		{
...
function validateInventoryForm() {
	...
	if ($("#inventoryName").val().trim() == "") {
		return "Insert Inventory Name.";
	}

	if ($("#inventoryType").val().trim() == "") {
		return "Insert Inventory Type.";
	}

	if ($("#inventoryStore").val().trim() == "") {
		return "Insert Inventory Store.";
	}

	if ($("#inventoryQuantity").val().trim() == "") {
		return "Insert Inventory Quantity.";
	}
